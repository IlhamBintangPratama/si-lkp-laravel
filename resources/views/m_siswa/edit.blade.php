...
<div class="p-8 rounded border border-gray-200">
    <h1 class="font-medium text-3xl">Edit Siswa</h1>
    ...
    <form method="Post" action="{{ url('/m_siswa/'.$siswa->id.'/update') }}" name="post" enctype="multipart/form-data">
        {{ csrf_field() }}
        <div class="mt-8 space-y-6">
            <div>
                ...
                focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full" value="{{ old('name', $siswa->nik) }}" required />
            </div><br>
            ...
            <div>
                ...
                focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full" value="{{ old('name', $siswa->nama) }}" required />
            </div><br>
            <div class="relative">
                ...
                <select class="form-control block appearance-none w-full bg-grey-200 border border-grey-200 text-grey-darker py-2 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-grey"
                    ...
                    <option value="{{ $gender->id }}" {{ $gender->id == $siswa->jenis_kelamin ? 'selected' : '' }}>{{ $gender->gender }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full" value="{{ old('name', $siswa->email) }}" required />
            </div><br>
            <div>
                <label for="no_hp" class="text-sm text-gray-700 block mb-1 font-medium">No. Telp</label>
                <input type="text" name="no_hp" id="no_hp" class="bg-gray-100 border border-gray-200 rounded py-1 px-3 block 
                focus:ring-blue-500 focus:border-blue-500 text-gray-700 w-full" value="{{ old('name', $siswa->no_hp) }}" required />
            </div>
        </div>
