...
<div class="flex flex-1  flex-col md:flex-row lg:flex-row mx-2">
    <div class="mb-2 border-solid border-gray-300 rounded border shadow-sm w-full">
        ...
        <div class="p-3">
            <div class="mb-3">
                <a class="bg-teal-300 hover:bg-teal-400 right-0 py-2 px-3 text-white rounded-lg float-right mb-3" href="{{ url('/m_siswa/create') }}">
            ...
            <table class="table-responsive w-full rounded">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="border px-4 py-2">{{ $siswa->firstItem()+$no }}</td>
                        <td class="border px-4 py-2">{{ $s->nik }}</td>
                        <td class="border px-4 py-2">{{ $s->nama }}</td>
                        <td class="border px-4 py-2">{{ $s->gender }}</td>
                        <td class="border px-4 py-2">{{ $s->email }}</td>
                        <td class="border px-4 py-2">{{ $s->no_hp }}</td>
                        
                        <td class="border px-4 py-2">
                            <a class="bg-teal-300 hover:bg-teal-400 cursor-pointer rounded p-2 mx-1 text-white" href="{{ url('/m_siswa/'.$s->id.'/show') }}">
                                    <i class="fas fa-eye"></i></a>
                            <a class="bg-teal-300 hover:bg-teal-400 cursor-pointer rounded p-2 mx-1 text-white" href="{{ url('/m_siswa/'.$s->id.'/edit') }}">
                                    <i class="fas fa-edit"></i></a>
                            <button type="button" onclick="return openModal({{ $s->id }})"
...
<script>
    ...
    function openModal(id){
        ...
        form.action = `{{url('m_siswa/${id}/destroy')}}`
    }
</script>
@endsection
