...
@section('content')
    @if ($message = Session::get('reseted'))
        <div id="notif" class="bg-green-300 mb-2 border border-green-300 text-green-600 px-4 py-3 rounded relative"
            role="alert">
    ...
    @if ($message = Session::get('warning'))
        <div id="notif" class="bg-orange-300 mb-2 border border-orange-300 text-orange-dark px-4 py-3 rounded relative"
            role="alert">
            <strong class="font-bold">Oops!</strong>
            <span class="block sm:inline">{{ $message }}</span>
            <span class="absolute top-0 bottom-0 right-0 px-4 py-3">
                <svg class="fill-current h-6 w-6 text-orange" role="button" xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 20 20">
                    <title>Close</title>
                    <path
                        d="M14.348 14.849a1.2 1.2 0 0 1-1.697 0L10 11.819l-2.651 3.029a1.2 1.2 0 1 1-1.697-1.697l2.758-3.15-2.759-3.152a1.2 1.2 0 1 1 1.697-1.697L10 8.183l2.651-3.031a1.2 1.2 0 1 1 1.697 1.697l-2.758 3.152 2.758 3.15a1.2 1.2 0 0 1 0 1.698z" />
                ...
                <table class="table-responsive w-full rounded">
                    <thead>
                        <tr>
                            ...
                            <th class="border w-1/3 px-4 py-2">Nama</th>
                    ...
                    <tbody>
                        @forelse ($user as $no => $u)
                            <tr>
                                <td class="border px-4 py-2">{{ $user->firstItem() + $no }}</td>
                                <td class="border px-4 py-2">{{ $u->name }}</td>
                                <td class="border px-4 py-2">{{ $u->email }}</td>
                                <td class="border px-4 py-2">{{ $u->role == 2 ? 'pendidik' : 'siswa ' }}</td>

                                <td class="border px-4 py-2">
                                    <a class="bg-teal-300 hover:bg-teal-400 cursor-pointer rounded p-2 mx-1 text-white"
                                        href="{{ url('/m_user/' . $u->id . '/show') }}">
                                        <i class="fas fa-eye"></i></a>
                                    <button type="button" onclick="return openModal({{ $u->id }})"
    ...
    <script>
        ...
        function openModal(id) {
            ...
            form.action = `{{ url('m_user/${id}/destroy') }}`
        }
    </script>
@endsection
