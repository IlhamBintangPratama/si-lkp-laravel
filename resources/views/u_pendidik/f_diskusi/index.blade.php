...
<div class="flex flex-1  flex-col md:flex-row lg:flex-row mx-2">
    <div class="mb-2 border-solid border-gray-300 rounded border shadow-sm w-full">
        ...
        <div class="p-3">
            <div class="mb-3">
                <a class="bg-teal-300 hover:bg-teal-400 right-0 py-2 px-3 text-white rounded-lg float-right mb-3" href="{{ url('/data_penilaian/create') }}">
            ...
            <table class="table-responsive w-full rounded">
                <thead>
                    <tr>
                        ...
                        <th class="border px-4 py-2" style="width: 22%">Forum</th>
                        <th class="border px-4 py-2" style="width: 22%">Komen</th>
                        <th class="border px-4 py-2" style="width: 14%">Updated</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($diskusi as $no => $s)
                    <tr>
                        <td class="border px-4 py-2">{{ $diskusi->firstItem()+$no }}</td>
                        <td class="border px-4 py-2">
                            <span class="font-bold text-lg">{{ $s->judul }}</span><br>
                            {{ $s->isi }}
                        </td>
                        <td class="border px-4 py-2">{{ count($s->id_komen) }}</td>
                        <td class="border px-4 py-2">{{ $s->nilai_kreatif }}</td>
                        <td class="border px-4 py-2">{{ $s->nilai_ketrampilan }}</td>
                        <td class="border px-4 py-2">{{ $s->nilai_sikap }}</td>
                        {{-- <td class="border px-4 py-2">{{ $s->created_at->format('D, Y-m-d') }}</td> --}}
                        
                        <td class="border px-4 py-2">
                            <a class="bg-teal-300 hover:bg-teal-400 cursor-pointer rounded p-2 mx-1 text-white" href="{{ url('/data_penilaian/'.$s->id.'/show') }}">
                                    <i class="fas fa-eye"></i></a>
                            <a class="bg-teal-300 hover:bg-teal-400 cursor-pointer rounded p-2 mx-1 text-white" href="{{ url('/data_penilaian/'.$s->id.'/edit') }}">
...
{{-- <script>
    ...
    function openModal(id){
        ...
        form.action = `{{url('data_penilaian/${id}/destroy')}}`
    }
</script> --}}
@endsection